...
public class CooldownInterceptor implements CommandInterceptor {
    ...
    public CooldownInterceptor(String commandName, int cooldownSeconds, I18nService i18n) {
        this(commandName, () -> new CommandPolicy(cooldownSeconds, false), i18n);
    }
     ...
     */
    public CooldownInterceptor(String commandName, Supplier<CommandPolicy> policy, I18nService i18n) {
    ...
    public Component preHandle(CommandSender sender, String ignored) {
        ...
        if (CooldownRegistry.isCoolingDown(key, Math.max(0, policy.get().cooldownSeconds()))) {
            return feedbackService.cooldownTip(sender);
        }
        return null;
    }
}
