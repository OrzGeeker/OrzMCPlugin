...
public final class PlayerNameRuleFeedback {
     ...
     * （也是持久化/移除时的键值）。非法类型/非法正则返回失败反馈，不触碰服务。</p>
     */
    public static Outcome feedback(AccessRuleService svc, String typeRaw, String valueRaw, boolean remove) {
        PlayerNameRule.ParsedRule parsed = PlayerNameRule.parse(typeRaw, valueRaw);
        if (!parsed.valid()) {
        ...
        if (remove) {
            return svc.removePlayerNameRule(parsed.type(), valueRaw)
                    ? new Outcome(true, "已移除玩家名规则: " + display)
                    : new Outcome(false, "未找到该玩家名规则: " + display);
        }
        svc.addPlayerNameRule(parsed.type(), valueRaw);
        return new Outcome(true, "已添加玩家名规则: " + display);
    }
}
