...
6. **开通消息权限（必须，真机验证）**：开发者后台找到机器人的权限/消息设置，确认：
   - **机器人可获取的群聊消息范围 = 获取群内全部消息**——否则插件收不到非 @ 的群消息（只发 @ 事件）；
   - **机器人主动在群聊内发言的开关 = 开启**——否则主动下行/广播会被拒（被动回复不受影响）；
7. **把机器人加进群**：手机 QQ → 测试群 → 群设置 → **群机器人** → 底部找到你的机器人 → 添加。添加后在群里 @机器人 即可收到消息。
...
| 群消息收不到（只收到 @ 事件） | 机器人权限「可获取的群聊消息范围」未设为“群内全部消息”——到开发者后台开启后重试（真机验证） |
| 主动下行/广播被拒 | 「机器人主动在群聊内发言」开关未开启——开发者后台开启（被动回复不受影响） |
| 连接后反复收到 op9（无效会话） | identify token 未带 `QQBot ` 前缀（插件已内置正确格式）；持续出现多为凭据/token 提前失效（11244） |
| 重连被平台限频（HTTP 400 code 100017） | /gateway/bot 有频率限制；插件已做 60s URL 缓存与重连退避，勿手工高频重启 |
