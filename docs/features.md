...
| § | 主题 |
|---|------|
| [一、白名单管理](#一白名单管理) | 强制白名单 / 踢出提示 / Bot 命令 / 不活跃清理 |
| [二、多平台 Bot 系统](#二多平台-bot-系统) | 平台支持 / Bot 命令 / 通知 / 健康 / 接入手册索引 |
| [三、跨服传送门](#三跨服传送门) | 传送门创建/删除/跨服 |
| [四、TNT 保护系统](#四tnt-保护系统) | 放置/重生锚/爆炸通知 |
| [五、安全与访问控制](#五安全与访问控制) | GeoIP / 访问规则 / guard / chat / 限流 / 加固 |
| [六、传送弓 🏹](#六传送弓-) | 获取 / 传送逻辑 / 实体策略 |
| [七、世界维护（备份与优化）](#七世界维护备份与优化) | 备份 / 优化 / 进度 / 维护体验 |
| [八、加入/退出/踢出通知](#八玩家加入退出踢出通知) | 推送内容 / 模板 |
| [九、新手指南书 📖](#九新手指南书-) | 自动发放 / 内容 |
| [十、运行时配置管理](#十运行时配置管理) | /config 命令 / 可配置项 |
| [十一、游戏内指令总表](#十一游戏内指令总表) | 指令速查 |
| [十二、OrzMC 菜单（开发中）](#十二orzmc-菜单开发中) | 箱子 GUI |
| [十三、基础设施能力](#十三基础设施能力) | 跨模块能力 |
| [十四、配置插件](#十四配置插件) | 配套插件说明 |
| [十五、权限管理系统](#十五权限管理系统rank--review) | LP 权限链 / 审核晋升 / prison |
| [十六、插件自更新](#十六插件自更新) | Hangar 检查更新 |
...
- **EasyBot 网关（默认，`backend: easybot`）**：外部网关服务统一接入 QQ、Telegram、Discord、飞书和微信；
  EasyBot 使用 WebSocket 向插件推送入站消息，插件通过 HTTP API 发送回复和服务器通知（手册 [`docs/manuals/bot-easybot.md`](manuals/bot-easybot.md)）；
...
### 2.5 接入手册（操作手册见 docs/manuals/）

> 完整的分步接入/验收手册已独立到 [`docs/manuals/`](manuals/README.md)：**先读** [`bot-builtin-common.md`](manuals/bot-builtin-common.md)
> （builtin 公共骨架：backend/im.yml/bind 命令/通用 FAQ），再接目标平台册。本清单只做概览与索引。
...
| · Telegram | [`bot-telegram.md`](manuals/bot-telegram.md) | BotFather bot（长轮询，免公网入站） |
| · Discord | [`bot-discord.md`](manuals/bot-discord.md) | 开发者门户 bot（Gateway WS） |

要点速记：

- **会话绑定**（三类）：`admin_group` 管理群 / `player_group` 玩家群（空则降级发管理群）/ `admin_dm` 管理员私聊——存于 `im_bindings.yml`，由 `/config im bind` 写入，**即时生效**；
- **绑定命令仅控制台/游戏内 op**（D10）；未绑定会话消息只进控制台日志与 `/config im status` 候选（D11，可直接复制 bind 命令）；
- **`im.yml` 改动（backend/凭据/代理）需重启生效**；一个平台凭据只允许一个实例消费事件（R3）；
- **网络代理**（D13，公共骨架 §4）：Telegram/Discord 国内服务器出墙必需；QQ/飞书海外部署可配代理回国；不配一律直连。
