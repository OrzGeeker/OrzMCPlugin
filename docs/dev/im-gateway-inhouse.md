...
1. **QQ 政策门槛**（最高风险，先于 P3a 排期核验）：开放平台注册（个人/企业）、机器人进群方式、沙箱→正式流程——owner 后台确认；**[2026-09-06 核验：✅ 已过——QQ builtin 真机三类会话闭环（手册 [`manuals/bot-qq.md`](../manuals/bot-qq.md)）]**
2. **飞书企业组织**：无企业资源则该平台实际不可用（需从 builtin 列表剔除或仅文档支持）；**[2026-09-06 核验：✅ 已过——企业租户真机三类会话闭环（手册 [`manuals/bot-feishu.md`](../manuals/bot-feishu.md)）]**
3. **字段演进**：EasyBot 0.0.33 事件 `sender.nickname/user_id` 与 main `sender.name/id` 不一致——若升级网关需适配（已在 EasyBot#114 文档落账）；**[2026-09-06：⏳ owner 跟进 EasyBot#114]**
4. **会话值兼容**：QQ 填 openid 而非群号（现有生产 easybot.yml 若填错则两模式都发不出，需核对现网值）；**[2026-09-06 核验：✅ 文档化——builtin 用平台原生标识，切 backend 需按 [`manuals/`](../manuals/README.md) 各平台册重绑定]**
5. **国内服务器 × TG/DC 可达性**（D13 落地形态待验）：代理/API 基址覆盖方案需在首个非国内平台 adapter（Discord/TG 顺序靠后）前定案；QQ/飞书不受影响；**[2026-09-06 核验：✅ 已定案落地——ImProxyConfig 四平台全覆盖（#323+#329）]；⏳ 海外真机验证见 §10-I1**
6. **QQ 被动窗口与主动配额数字**（D14）：被动回复窗口时长、主动消息频次配额、文本长度上限需按官方文档/沙箱实测核验后固化进常量表（R7）——直接决定一问一答超时处理与广播节流参数。**[2026-09-06：⏳ 遗留见 §10-I2]**

## 10. 遗留跟进清单（2026-09-06 快照，四平台落地后）

> 后续会话 / 低成本模型凭此续作，不回读旧对话（AGENTS 红线 8「交接文件」同款卡格式）。
> 卡字段：目标 / 改动点 / 验收 / 状态。涉及行为/配置变更的卡合入时同步手册（§2.6–§2.9）与 CHANGELOG。

### P1 真机验证类（需特定环境）

| 卡 | 目标 | 改动点 | 验收 | 状态 |
|----|------|--------|------|------|
| I1 | QQ/飞书 proxy 海外真机 | #329 已实现四平台 proxy（海外部署代理回国）；海外直连国内 API 场景本机无法复现 | 海外服务器 im.yml 配 proxy → `[qq]/[feishu]` READY + 双向收发 | ⏳ 待海外部署实测 |
| I2 | QQ R7 数字固化（D14） | 被动回复窗口时长 / 主动消息频次配额 / 单条文本上限 → 沙箱实测后固化常量表；`manuals/bot-qq.md` 能力边界去掉「待实测固化」注记 | 常量表落地 + 手册同步 | ⏳ 待 QQ 沙箱 |
...
- 真机凭据（QQ/TG/DC/飞书 token、测试服务器/群/channel id）与本地 im.yml/im_bindings.yml
- pi 本地补丁文件与验证进度、调试手段（screen 会话、RCON 限制、screen stuff 交互、`run/logs/latest.log`）
