...
1. **QQ 政策门槛**（最高风险，先于 P3a 排期核验）：开放平台注册（个人/企业）、机器人进群方式、沙箱→正式流程——owner 后台确认；**[2026-09-06 核验：✅ 已过——QQ builtin 真机三类会话闭环（§2.6）]**
2. **飞书企业组织**：无企业资源则该平台实际不可用（需从 builtin 列表剔除或仅文档支持）；**[2026-09-06 核验：✅ 已过——企业租户真机三类会话闭环（§2.7）]**
...
4. **会话值兼容**：QQ 填 openid 而非群号（现有生产 easybot.yml 若填错则两模式都发不出，需核对现网值）；**[2026-09-06 核验：✅ 文档化——builtin 用平台原生标识，切 backend 需按 §2.6–§2.9 重绑定]**
...
| I2 | QQ R7 数字固化（D14） | 被动回复窗口时长 / 主动消息频次配额 / 单条文本上限 → 沙箱实测后固化常量表；features.md §2.6 能力边界去掉「待实测固化」注记 | 常量表落地 + 手册同步 | ⏳ 待 QQ 沙箱 |
