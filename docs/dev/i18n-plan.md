...
> **状态：方案定稿（D1/D3/D7 已拍板；其余决策见 §4 待确认列）**｜**最后更新**：2026-09-16
> **范围**：一期英文 + 中文；架构上预留第三语言扩展（新增语言 = 加一个 yml 资源，业务代码零改动）。
> **配套**：本方案为计划文档，不含代码改动；拍板后按 §5 拆实施 PR（先 P0 基础设施）。
...
| D2 | 默认语言 | zh-CN（老服零改动、兼容现网） | 待确认（推荐） |
| D3 | 渲染模型 | A1 文案抽取（不引入 MiniMessage；复用 TemplateRenderer） | ✅ 已拍板 |
| D4 | 语言包形态 | **内置资源（bundled）+ 可选数据目录覆盖层（custom）**（修订原「数据目录同名文件」方案，免版本同步坑） | 待确认（推荐） |
| D5 | 兜底 | 未装码/缺 key → default_lang → zh-CN → key 本体 | 待确认（推荐） |
| D6 | 游戏内语言 | 跟随客户端 locale（Paper `Player.locale()`），不做 per-player 手动切换 | 待确认（推荐） |
| D7 | 英文来源 | AI 初译 + owner 校对，随各域 PR 分批交付 | ✅ 已拍板 |
| D8 | 事件通知多语言 | R1（default_lang 渲染一次）；R2 延迟渲染二期 | 待确认（推荐） |
| D9 | 样式纯化 | `OrzTextStyles`/`OrzConstants` 内嵌文案迁出，样式类不含任何文案 | 待确认（推荐） |
| D10 | 事件正文 key | 平移为 `event.<name>`，`TemplateKeys` 同步改名并收口散落字面量 | 待确认（推荐） |
| D11 | custom 空值语义 | 覆盖值为 `""` = 屏蔽该消息 | 待确认（推荐） |
