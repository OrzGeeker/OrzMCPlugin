...
14. **长时任务连续执行（2026-09-16 增补）**：需求方授权「CI 绿后合入即继续，直到完成全部」的长跑任务（如 i18n 逐域迁移），agent **无需逐轮确认**：每域/每卡一个独立小 PR → CI 绿即 squash 合入 develop → 基于最新 develop 开下一卡。纪律：
    - **顺序链合入防冲突**：多个 PR 若改动同一批文件（如语言包 / MessageKeys 等尾部追加文件），必须**串行链式**合入（每 PR 基于最新 develop 拉分支），禁止并行攒批后统一合——文件尾 hunk 会冲突，rebase 往返更贵；
    - **轮次预算与收口**：每轮（单回合）默认产出 1–2 个功能 PR；到预算或上下文变长（token 成本信号）即**收口**——不新开卡，只提交/开 PR/等 CI 绿合入 + 更新交接文档 + 阶段汇报，然后停下等下一轮指令；
    - **批量轮询**：串行等 CI 用一次 sleep ~2.5min 后批量 `gh pr checks`（多 PR 并行时同规则），不许逐 PR 高频空转。
15. **交接文档 + 上下文压缩（2026-09-16 增补，跨会话/压缩续作单一事实源）**：长任务开跑即建交接文件并保持最新；上下文压缩或换会话后**一律凭交接文件续作，不回读旧对话、不凭记忆复述**（旧细节只从交接/文档/git 取）。交接文档位置：仓库内 `docs/dev/<task>-handoff.md`（随 PR 可见，推荐）或本机 `~/<task>-handoff.md`（涉密/不随仓库）；第一版交接声明所用位置。模板（必备块）：
```markdown
# <任务> 交接
> 状态：现行 ｜ 最后更新：YYYY-MM-DD
## 任务与目标
一句话 + 完成验收定义（引用方案文档编号/PR）。
## 已完成（按时间倒序）
- PR #xxx 描述结果；PR #yyy …
## 进行中卡
- 分支：feature/<名>（基 origin/develop）
- 改动范围 / 已完成步骤 / 下一步动作
- 风险：如「语言包尾部追加，PR 必须串行合入」
## 未完成清单（按依赖排序）
## 下一棒开场指令
「读 <交接路径>，从卡 X 开始：checkout 分支 <名>，完成 Y，验收 Z；`spotlessApply && :test --tests …`；CI 绿合入后继续卡 X+1」
```
